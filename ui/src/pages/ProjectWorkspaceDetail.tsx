...
/** Source-type options carry i18n keys; callers resolve them with `t()`. */
const SOURCE_TYPE_OPTIONS: Array<{ value: ProjectWorkspaceSourceType; labelKey: string; descriptionKey: string }> = [
  { value: "local_path", labelKey: "workspaces.projectDetail.sourceTypes.localPathLabel", descriptionKey: "workspaces.projectDetail.sourceTypes.localPathDesc" },
  { value: "non_git_path", labelKey: "workspaces.projectDetail.sourceTypes.nonGitPathLabel", descriptionKey: "workspaces.projectDetail.sourceTypes.nonGitPathDesc" },
  { value: "git_repo", labelKey: "workspaces.projectDetail.sourceTypes.gitRepoLabel", descriptionKey: "workspaces.projectDetail.sourceTypes.gitRepoDesc" },
  { value: "remote_managed", labelKey: "workspaces.projectDetail.sourceTypes.remoteManagedLabel", descriptionKey: "workspaces.projectDetail.sourceTypes.remoteManagedDesc" },
];

/** Visibility options carry i18n keys; callers resolve them with `t()`. */
const VISIBILITY_OPTIONS: Array<{ value: ProjectWorkspaceVisibility; labelKey: string }> = [
  { value: "default", labelKey: "workspaces.projectDetail.visibilityOptions.default" },
  { value: "advanced", labelKey: "workspaces.projectDetail.visibilityOptions.advanced" },
];
...
function parseRuntimeConfigJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: "workspaces.projectDetail.errors.runtimeJsonObject",
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : "workspaces.projectDetail.errors.invalidJson",
    };
  }
}
...
function validateWorkspaceForm(form: WorkspaceFormState) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return "workspaces.projectDetail.errors.remoteManagedRequired";
    }
  } else if (!cwd && !repoUrl) {
    return "workspaces.projectDetail.errors.localOrRepoRequired";
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return "workspaces.projectDetail.errors.absolutePathRequired";
  }
  ...
  if (repoUrl) {
    try {
      ...
      return "workspaces.projectDetail.errors.repoUrlInvalid";
    }
  }
...
export function ProjectWorkspaceDetail() {
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : "workspaces.projectDetail.errors.saveFailed");
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : "workspaces.projectDetail.errors.updateFailed");
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("workspaces.projectDetail.runtimeMessages.jobCompleted")
          : request.action === "stop"
            ? t("workspaces.projectDetail.runtimeMessages.serviceStopped")
            : request.action === "restart"
              ? t("workspaces.projectDetail.runtimeMessages.serviceRestarted")
              : t("workspaces.projectDetail.runtimeMessages.serviceStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : "workspaces.projectDetail.errors.controlFailed");
    },
  });

  if (projectQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("workspaces.projectDetail.loading")}</p>;
  if (projectQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {projectQuery.error instanceof Error ? projectQuery.error.message : t("workspaces.projectDetail.loadError")}
      </p>
    );
  }
  if (!project || !workspace || !form || !initialState) {
    return <p className="text-sm text-muted-foreground">{t("workspaces.projectDetail.notFound")}</p>;
  }
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link to={`/projects/${canonicalProjectRef}/workspaces`}>
            ...
            {t("workspaces.projectDetail.backToWorkspaces")}
          </Link>
        </Button>
        <div className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-xs text-muted-foreground">
          {workspace.isPrimary
            ? t("workspaces.projectDetail.primaryBadge")
            : t("workspaces.projectDetail.secondaryBadge")}
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
              <div className="space-y-2">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                  {t("workspaces.projectDetail.kicker")}
                </div>
                <h1 className="text-2xl font-semibold">{workspace.name}</h1>
                <p className="max-w-2xl text-sm text-muted-foreground">
                  {t("workspaces.projectDetail.description")}
                </p>
              </div>
              {!workspace.isPrimary ? (
                <Button
                ...
                >
                  ...
                  {t("workspaces.projectDetail.makePrimary")}
                </Button>
              ) : (
                <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
                  ...
                  {t("workspaces.projectDetail.primaryNote")}
                </div>
              )}
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label={t("workspaces.projectDetail.fields.name")}>
                <input
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                  value={form.name}
                  onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                  placeholder={t("workspaces.projectDetail.fields.namePlaceholder")}
                />
              </Field>

              <Field label={t("workspaces.projectDetail.fields.visibility")}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("workspaces.projectDetail.fields.sourceType")} hint={sourceTypeDescriptionKey ? t(sourceTypeDescriptionKey) : undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("workspaces.projectDetail.fields.localPath")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.cwd}
                    onChange={(event) => setForm((current) => current ? { ...current, cwd: event.target.value } : current)}
                    placeholder={t("workspaces.projectDetail.fields.localPathPlaceholder")}
                  />
                </Field>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.projectDetail.fields.repoUrl")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
                    onChange={(event) => setForm((current) => current ? { ...current, repoUrl: event.target.value } : current)}
                    placeholder={t("workspaces.projectDetail.fields.repoUrlPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.projectDetail.fields.repoRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
                    onChange={(event) => setForm((current) => current ? { ...current, repoRef: event.target.value } : current)}
                    placeholder={t("workspaces.projectDetail.fields.repoRefPlaceholder")}
                  />
                </Field>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.projectDetail.fields.defaultRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.defaultRef}
                    onChange={(event) => setForm((current) => current ? { ...current, defaultRef: event.target.value } : current)}
                    placeholder={t("workspaces.projectDetail.fields.defaultRefPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.projectDetail.fields.sharedWorkspaceKey")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    ...
                    placeholder={t("workspaces.projectDetail.fields.sharedWorkspaceKeyPlaceholder")}
                  />
                </Field>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.projectDetail.fields.remoteProvider")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.remoteProvider}
                    onChange={(event) => setForm((current) => current ? { ...current, remoteProvider: event.target.value } : current)}
                    placeholder={t("workspaces.projectDetail.fields.remoteProviderPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.projectDetail.fields.remoteWorkspaceRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    ...
                    placeholder={t("workspaces.projectDetail.fields.remoteWorkspaceRefPlaceholder")}
                  />
                </Field>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.projectDetail.fields.setupCommand")} hint={t("workspaces.projectDetail.fields.setupCommandHint")}>
                  <textarea
                    ...
                    placeholder={t("workspaces.projectDetail.fields.setupCommandPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.projectDetail.fields.cleanupCommand")} hint={t("workspaces.projectDetail.fields.cleanupCommandHint")}>
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.cleanupCommand}
                    onChange={(event) => setForm((current) => current ? { ...current, cleanupCommand: event.target.value } : current)}
                    placeholder={t("workspaces.projectDetail.fields.cleanupCommandPlaceholder")}
                  />
                </Field>
              </div>

              <details className="rounded-xl border border-dashed border-border/70 bg-background px-3 py-3">
                <summary className="cursor-pointer text-sm font-medium">{t("workspaces.projectDetail.advancedJson.summary")}</summary>
                <p className="mt-2 text-sm text-muted-foreground">
                  {t("workspaces.projectDetail.advancedJson.description")}
                </p>
                <div className="mt-3">
                  <Field label={t("workspaces.projectDetail.advancedJson.label")} hint={t("workspaces.projectDetail.advancedJson.hint")}>
                    <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                ...
                {t("workspaces.projectDetail.saveChanges")}
              </Button>
              <Button
                variant="outline"
                className="w-full sm:w-auto"
              ...
              >
                {t("workspaces.projectDetail.reset")}
              </Button>
              ...
              {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("workspaces.projectDetail.noUnsavedChanges")}</p> : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces.projectDetail.facts.kicker")}</div>
              <h2 className="text-lg font-semibold">{t("workspaces.projectDetail.facts.title")}</h2>
            </div>
            <Separator className="my-4" />
            <DetailRow label={t("workspaces.projectDetail.facts.project")}>
              <Link to={`/projects/${canonicalProjectRef}`} className="hover:underline">{project.name}</Link>
            </DetailRow>
            <DetailRow label={t("workspaces.projectDetail.facts.workspaceId")}>
              <span className="break-all font-mono text-xs">{workspace.id}</span>
            </DetailRow>
            <DetailRow label={t("workspaces.projectDetail.facts.localPath")}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("common.none")}</span>
            </DetailRow>
            <DetailRow label={t("workspaces.projectDetail.facts.repo")}>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces.projectDetail.commands.kicker")}</div>
                <h2 className="text-lg font-semibold">{t("workspaces.projectDetail.commands.title")}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("workspaces.projectDetail.commands.description")}
                </p>
              </div>
            </div>
            <WorkspaceRuntimeControls
              ...
              serviceEmptyMessage={
                workspace.runtimeConfig?.workspaceRuntime
                  ? t("workspaces.projectDetail.commands.serviceEmptyWithConfig")
                  : t("workspaces.projectDetail.commands.serviceEmptyNoConfig")
              }
              jobEmptyMessage={t("workspaces.projectDetail.commands.jobEmpty")}
              disabledHint={t("workspaces.projectDetail.commands.disabledHint")}
