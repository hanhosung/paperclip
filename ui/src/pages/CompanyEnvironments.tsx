...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("companySettings.environments.breadcrumb") },
    ]);
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId
          ? t("companySettings.environments.toast.updated")
          : t("companySettings.environments.toast.created"),
        body: t("companySettings.environments.toast.ready", { name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companySettings.environments.toast.saveFailed"),
        body: error instanceof Error ? error.message : t("companySettings.environments.toast.saveFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok
          ? t("companySettings.environments.toast.probePassed")
          : t("companySettings.environments.toast.probeFailed"),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      setProbeResults((current) => ({
        ...
        [environmentId]: {
          ...
          summary: error instanceof Error ? error.message : t("companySettings.environments.toast.probeFailedBody"),
          details: null,
        },
      }));
      pushToast({
        title: t("companySettings.environments.toast.probeFailed"),
        body: error instanceof Error ? error.message : t("companySettings.environments.toast.probeFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok
          ? t("companySettings.environments.toast.draftProbePassed")
          : t("companySettings.environments.toast.draftProbeFailed"),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companySettings.environments.toast.draftProbeFailed"),
        body: error instanceof Error ? error.message : t("companySettings.environments.toast.probeFailedBody"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("companySettings.environments.selectCompany")}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("companySettings.environments.title")}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("companySettings.environments.disabled")}
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("companySettings.environments.description")}
        </p>
      </div>

      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("companySettings.environments.matrixNote")}
        </div>
        {sandboxCreationEnabled ? (
          <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            {t("companySettings.environments.installedProvidersPrefix")}{" "}
            <span className="font-medium text-foreground">
              {discoveredPluginSandboxProviders.map((provider) => provider.displayName).join(", ")}
            </span>
            {t("companySettings.environments.installedProvidersSuffix")}
          </div>
        ) : null}

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("companySettings.environments.supportCaption")}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("companySettings.environments.colAdapter")}</th>
                <th className="px-3 py-2 font-medium">{t("companySettings.environments.colLocal")}</th>
                <th className="px-3 py-2 font-medium">{t("companySettings.environments.colSsh")}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("companySettings.environments.colSandbox")}</th>
                ) : null}
              </tr>
            </thead>
              ...
              {(environmentCapabilities?.adapters.map((support) => ({
                ...
                <tr key={adapterType}>
                  ...
                  <td className="px-3 py-2">
                    <SupportMark
                      supported={support.drivers.local === "supported"}
                      yesLabel={t("companySettings.environments.supportYes")}
                      noLabel={t("companySettings.environments.supportNo")}
                    />
                  </td>
                  <td className="px-3 py-2">
                    <SupportMark
                      supported={support.drivers.ssh === "supported"}
                      yesLabel={t("companySettings.environments.supportYes")}
                      noLabel={t("companySettings.environments.supportNo")}
                    />
                  </td>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("companySettings.environments.empty")}</div>
          ) : (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      {environment.driver === "ssh" ? (
                        <div className="text-xs text-muted-foreground">
                          {typeof environment.config.host === "string"
                            ...
                            : t("companySettings.environments.sshHost")}{" "}
                          ·{" "}
                          {typeof environment.config.username === "string"
                            ? environment.config.username
                            : t("companySettings.environments.sshUser")}
                        </div>
                          ...
                          {(() => {
                            const provider =
                              typeof environment.config.provider === "string"
                                ...
                                : t("companySettings.environments.sandboxFallback");
                            const displayName =
                            ...
                            return `${displayName} ${t("companySettings.environments.sandboxProviderSuffix")}${summary ? ` · ${summary}` : ""}`;
                          })()}
                        </div>
                      ) : (
                        <div className="text-xs text-muted-foreground">{t("companySettings.environments.runsOnHost")}</div>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {environment.driver !== "local" ? (
                        ...
                        >
                          {environmentProbeMutation.isPending
                            ? t("companySettings.environments.testing")
                            : environment.driver === "ssh"
                              ? t("companySettings.environments.testConnection")
                              : t("companySettings.environments.testProvider")}
                        </Button>
                      ) : null}
                      <Button
                        size="sm"
                      ...
                      >
                        {isEditing
                          ? t("companySettings.environments.editing")
                          : t("companySettings.environments.edit")}
                      </Button>
                    </div>
                  </div>
                  {probe ? (
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId
              ? t("companySettings.environments.editEnvironment")
              : t("companySettings.environments.addEnvironment")}
          </div>
          <div className="space-y-3">
            <Field
              label={t("companySettings.environments.nameLabel")}
              hint={t("companySettings.environments.nameHint")}
            >
              <input
            ...
            <Field
              label={t("companySettings.environments.descriptionLabel")}
              hint={t("companySettings.environments.descriptionHint")}
            >
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field
              label={t("companySettings.environments.driverLabel")}
              hint={t("companySettings.environments.driverHint")}
            >
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              >
                <option value="ssh">{t("companySettings.environments.driverSsh")}</option>
                {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                  <option value="sandbox">{t("companySettings.environments.driverSandbox")}</option>
                ) : null}
                <option value="local">{t("companySettings.environments.driverLocal")}</option>
              </select>
            </Field>
            ...
            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field
                  label={t("companySettings.environments.sshHostLabel")}
                  hint={t("companySettings.environments.sshHostHint")}
                >
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field
                  label={t("companySettings.environments.sshPortLabel")}
                  hint={t("companySettings.environments.sshPortHint")}
                >
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="number"
                ...
                <Field
                  label={t("companySettings.environments.sshUsernameLabel")}
                  hint={t("companySettings.environments.sshUsernameHint")}
                >
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field
                  label={t("companySettings.environments.sshRemotePathLabel")}
                  hint={t("companySettings.environments.sshRemotePathHint")}
                >
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                    placeholder={t("companySettings.environments.sshRemotePathPlaceholder")}
                ...
                <Field
                  label={t("companySettings.environments.sshPrivateKeyLabel")}
                  hint={t("companySettings.environments.sshPrivateKeyHint")}
                >
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    ...
                    >
                      <option value="">{t("companySettings.environments.noSavedSecret")}</option>
                      {(secrets ?? []).map((secret) => (
                ...
                <Field
                  label={t("companySettings.environments.sshKnownHostsLabel")}
                  hint={t("companySettings.environments.sshKnownHostsHint")}
                >
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("companySettings.environments.sshStrictLabel")}
                    hint={t("companySettings.environments.sshStrictHint")}
            ...
            {environmentForm.driver === "sandbox" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field
                  label={t("companySettings.environments.sandboxProviderLabel")}
                  hint={t("companySettings.environments.sandboxProviderHint")}
                >
                  <select
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                      {t("companySettings.environments.noSandboxConfig")}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {environmentMutation.isPending
                  ? editingEnvironmentId
                    ? t("companySettings.environments.saving")
                    : t("companySettings.environments.creating")
                  : editingEnvironmentId
                    ? t("companySettings.environments.saveEnvironment")
                    : t("companySettings.environments.createEnvironment")}
              </Button>
              {editingEnvironmentId ? (
                <Button
                  size="sm"
                ...
                >
                  {t("companySettings.environments.cancel")}
                </Button>
              ) : null}
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending
                    ? t("companySettings.environments.testing")
                    : t("companySettings.environments.testDraft")}
                </Button>
              ) : null}
              {environmentMutation.isError ? (
                <span className="text-xs text-destructive">
                  {environmentMutation.error instanceof Error
                    ...
                    : t("companySettings.environments.toast.saveFailed")}
                </span>
              ) : null}
