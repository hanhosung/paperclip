  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-label={t("projects.detail.changeColor")}
      />
      {open && (
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {PROJECT_COLORS.map((color) => (
              <button
                ...
                aria-label={t("projects.detail.selectColor", { color })}
              />
            ))}
          </div>
        </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("projects.detail.fallbackName");
      if (archived) {
        pushToast({ title: t("projects.detail.archivedToast", { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("projects.detail.unarchivedToast", { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? t("projects.detail.fallbackName") },
    ]);
  ...
  const projectBudgetSummary = useMemo(() => {
    ...
    return {
      ...
      scopeName: project?.name ?? t("projects.detail.fallbackName"),
      metric: "billed_cents",
      windowKind: "lifetime",
      amount: 0,
      observedAmount: 0,
