...
export function CompanyExport() {
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.export.exportFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.export.exportFailedLoadBody"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyData.export.downloadedTitle"),
        body: t("companyData.export.downloadedBody", { count: resultCheckedFiles.size, name: result.rootPath }),
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.export.exportFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.export.exportFailedBuildBody"),
      });
    },
  });
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {`${selectedCompany?.name ?? t("companyData.export.companyFallback")} ${t("companyData.export.exportSuffix")}`}
            </span>
            <span className="text-muted-foreground">
              {t("companyData.export.selectedSummary", { selected: selectedCount, total: totalFiles })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t("companyData.export.warningCount", { count: warnings.length })}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ? t("companyData.export.building")
              : t("companyData.export.exportButton", { count: selectedCount })}
          </Button>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("companyData.export.showMoreIssues", { visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>
