            ...
            <>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.inferenceLedger.budget", { amount: formatCents(spendData.summary.budgetCents) })
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs.byAgent.inOut", { in: formatTokens(row.inputTokens + row.cachedInputTokens), out: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costs.byAgent.apiRuns", { count: row.apiRunCount })}
                                    {" · "}
                                    ...
                                    >
                                      <div className="min-w-0">
                                        ...
                                        <div className="truncate text-muted-foreground">
                                          {providerDisplayName(modelRow.biller)} · {t(billingTypeKey(modelRow.billingType))}
                                        </div>
                                      </div>
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("costs.unit.tok", { tokens: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.financeTimeline.emptyOverview")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.budgets.activeIncidents")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs.budgets.incidentsHelp")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`costs.budgets.${scopeType}Budgets`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {t(`costs.budgets.${scopeType}Desc`)}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.byBiller.title")}</CardTitle>
                      <CardDescription>{t("costs.byBiller.description")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.byBiller.empty")}</p>
                      ) : (
