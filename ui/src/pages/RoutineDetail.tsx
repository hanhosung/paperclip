...
import {
  routineConcurrencyPolicyKey,
  routineCatchUpPolicyKey,
  routineRunStatusKey,
    ...
    <div className="rounded-lg border border-border p-4 space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("routines.detail.label")}</Label>
          <Input
            value={draft.label}
        ...
        {trigger.kind === "schedule" && (
          <div className="md:col-span-2 space-y-1.5">
            <Label className="text-xs">{t("routines.detail.schedule")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routines.detail.signingMode")}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routines.detail.replayWindow")}</Label>
                <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("routines.detail.lastResult", { result: trigger.lastResult })}</span>}
        <div className="ml-auto flex items-center gap-2">
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("routines.detail.rotateSecret")}
            </Button>
          )}
          <Button
            variant="outline"
...
export function RoutineDetail() {
  ...
  const dirtyFields = useMemo<RoutineHistoryDirtyFieldDescriptor[]>(() => {
    ...
    if (editDraft.title !== routineDefaults.title) {
      result.push({ key: "title", label: t("routines.detail.dirtyField.title") });
    }
    if (editDraft.description !== routineDefaults.description) {
      result.push({ key: "description", label: t("routines.detail.dirtyField.description") });
    }
    if (editDraft.projectId !== routineDefaults.projectId) {
      result.push({ key: "projectId", label: t("routines.detail.dirtyField.project") });
    }
    if (editDraft.assigneeAgentId !== routineDefaults.assigneeAgentId) {
      result.push({ key: "assigneeAgentId", label: t("routines.detail.dirtyField.defaultAgent") });
    }
    if (editDraft.priority !== routineDefaults.priority) {
      result.push({ key: "priority", label: t("routines.detail.dirtyField.priority") });
    }
    if (editDraft.concurrencyPolicy !== routineDefaults.concurrencyPolicy) {
      result.push({ key: "concurrencyPolicy", label: t("routines.detail.dirtyField.concurrencyPolicy") });
    }
    if (editDraft.catchUpPolicy !== routineDefaults.catchUpPolicy) {
      result.push({ key: "catchUpPolicy", label: t("routines.detail.dirtyField.catchUpPolicy") });
    }
    if (JSON.stringify(editDraft.variables) !== JSON.stringify(routineDefaults.variables)) {
      result.push({ key: "variables", label: t("routines.detail.dirtyField.variables") });
    }
    return result;
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({ title: t("routines.toast.copied", { label }), tone: "success" });
    } catch (error) {
      pushToast({
        title: t("routines.toast.copyFailed", { label: label.toLowerCase() }),
        body: error instanceof Error ? error.message : t("routines.toast.clipboardError"),
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          title: t("routines.toast.changedTitle"),
          body: t("routines.toast.changedBody"),
          tone: "warn",
        });
        return;
      }
      pushToast({
        title: t("routines.toast.saveFailedTitle"),
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routines.toast.runStartedTitle"), tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        title: t("routines.toast.runFailedTitle"),
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("routines.toast.savedTitle"),
    ...
    onError: (error) => {
      pushToast({
        title: t("routines.toast.updateFailedTitle"),
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("routines.toast.webhookCreatedTitle"),
          entries: [{
        ...
        pushToast({
          title: t("routines.toast.triggerAddedTitle"),
    ...
    onError: (error) => {
      pushToast({
        title: t("routines.toast.addTriggerFailedTitle"),
        body: error instanceof Error ? error.message : t("routines.toast.addTriggerFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("routines.toast.triggerSavedTitle"),
    ...
    onError: (error) => {
      pushToast({
        title: t("routines.toast.updateTriggerFailedTitle"),
        body: error instanceof Error ? error.message : t("routines.toast.updateTriggerFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("routines.toast.triggerDeletedTitle"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("routines.toast.deleteTriggerFailedTitle"),
        body: error instanceof Error ? error.message : t("routines.toast.deleteTriggerFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("routines.toast.secretRotatedTitle"),
        entries: [{
          webhookUrl: result.secretMaterial.webhookUrl,
          webhookSecret: result.secretMaterial.webhookSecret,
        }],
    ...
    onError: (error) => {
      pushToast({
        title: t("routines.toast.rotateSecretFailedTitle"),
        body: error instanceof Error ? error.message : t("routines.toast.rotateSecretFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const automationLabel = routine.status === "archived"
    ? t("routines.detail.automationArchived")
    : !routine.assigneeAgentId
      ? t("routines.detail.automationDraft")
      : automationEnabled
        ? t("routines.detail.automationActive")
        : t("routines.detail.automationPaused");
    ...
    <div className="max-w-2xl space-y-6">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {routine.managedByPlugin ? (
            <Badge variant="outline" className="gap-1 text-xs text-muted-foreground">
              {t("routines.detail.managedBy", { name: routine.managedByPlugin.pluginDisplayName })}
          ...
          <ToggleSwitch
            ...
            onCheckedChange={() => {
              if (!automationEnabled && !routine.assigneeAgentId) {
                pushToast({
                  title: t("routines.toast.defaultAgentRequiredTitle"),
                  body: t("routines.toast.defaultAgentRequiredBody"),
                  tone: "warn",
                });
                return;
              }
            ...
            aria-label={automationEnabled ? t("routines.detail.pauseTriggersAria") : t("routines.detail.enableTriggersAria")}
          />
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("routines.detail.secretSaveNow")}</p>
          </div>
              ...
              <div key={`${entry.webhookUrl}-${index}`} className="space-y-2">
                {secretMessage.entries.length > 1 && (
                  <p className="text-xs font-medium text-muted-foreground">
                    {t("routines.detail.secretWebhookTrigger", { index: index + 1, total: secretMessage.entries.length })}
                  </p>
                )}
                <div className="flex items-center gap-2">
                  <Input value={entry.webhookUrl} readOnly className="flex-1" />
                  <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routines.toast.webhookUrlLabel"), entry.webhookUrl)}>
                    <Copy className="h-3.5 w-3.5 mr-1" />
                    {t("routines.detail.copyUrl")}
                  </Button>
                </div>
                <div className="flex items-center gap-2">
                  <Input value={entry.webhookSecret} readOnly className="flex-1" />
                  <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routines.toast.webhookSecretLabel"), entry.webhookSecret)}>
                    <Copy className="h-3.5 w-3.5 mr-1" />
                    {t("routines.detail.copySecret")}
                  </Button>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="font-medium text-amber-200">{t("routines.detail.conflictTitle")}</p>
              <p className="text-xs text-muted-foreground">
                {t("routines.detail.conflictBody")}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button
              ...
              >
                {t("routines.detail.reloadLatest")}
              </Button>
            </div>
          </div>
        </div>
      ...
      {!routine.assigneeAgentId ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-900 dark:text-amber-200">
          {t("routines.detail.defaultAgentRequired")}
        </div>
      ) : null}
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          <span>{t("routines.detail.for")}</span>
          <InlineEntitySelector
            ref={assigneeSelectorRef}
            value={editDraft.assigneeAgentId}
            options={assigneeOptions}
            recentOptionIds={recentAssigneeIds}
            placeholder={t("routines.detail.assignee")}
            ...
            renderTriggerValue={(option) =>
              option ? (
                ...
                <span className="text-muted-foreground">{t("routines.detail.assignee")}</span>
              )
            }
            renderOption={(option) => {
          ...
          />
          <span>{t("routines.detail.in")}</span>
          <InlineEntitySelector
            ref={projectSelectorRef}
            value={editDraft.projectId}
            options={projectOptions}
            recentOptionIds={recentProjectIds}
            placeholder={t("routines.detail.project")}
            ...
            renderTriggerValue={(option) =>
              option && currentProject ? (
                ...
                <span className="text-muted-foreground">{t("routines.detail.project")}</span>
              )
            }
            renderOption={(option) => {
              if (!option.id) return <span className="truncate">{option.label}</span>;
      ...
      <MarkdownEditor
        ...
        placeholder={t("routines.detail.instructionsPlaceholder")}
        bordered={false}
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("routines.detail.advancedTitle")}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.detail.concurrency")}</p>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {concurrencyPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{t(routineConcurrencyPolicyKey(value))}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{t(`routines.detail.concurrencyDesc.${editDraft.concurrencyPolicy}`)}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.detail.catchUp")}</p>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {catchUpPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{t(routineCatchUpPolicyKey(value))}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{t(`routines.detail.catchUpDesc.${editDraft.catchUpPolicy}`)}</p>
            </div>
          </div>
        </CollapsibleContent>
      </Collapsible>
...
<TabsTrigger value="activity" className="gap-1.5">
          ...
          <TabsTrigger value="history" className="gap-1.5">
            ...
            {t("routines.detail.tabs.history")}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("routines.detail.addTrigger")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routines.detail.kind")}</Label>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {kind}{kind === "webhook" ? t("routines.detail.kindComingSoon") : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">{t(`routines.detail.signingModeDesc.${newTrigger.signingMode}`)}</p>
                  </div>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? t("routines.detail.adding") : t("routines.detail.addTrigger")}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routines.detail.noTriggers")}</p>
          ) : (
            <div className="space-y-3">
                ...
                <div key={run.id} className="flex items-center justify-between px-3 py-2 text-sm">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <Badge variant={run.status === "failed" ? "destructive" : "secondary"} className="shrink-0">
                      {t(routineRunStatusKey(run.status))}
                    </Badge>
                    {run.trigger && (
              ...
              if (response.secretMaterials.length > 0) {
                setSecretMessage({
                  title: response.secretMaterials.length === 1
                    ? t("routines.toast.webhookRestoredTitle")
                    : t("routines.toast.webhookRestoredManyTitle", { count: response.secretMaterials.length }),
