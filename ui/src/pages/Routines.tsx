...
/** Group-label fallbacks. Defaults are English so the function stays usable in tests. */
...
const DEFAULT_ROUTINE_GROUP_LABELS: RoutineGroupLabels = {
...
export function buildRoutineGroups(
  ...
  labels: RoutineGroupLabels = DEFAULT_ROUTINE_GROUP_LABELS,
): RoutineGroup[] {
...
export function Routines() {
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.toast.createdTitle"),
        body: routine.assigneeAgentId
          ? t("routines.toast.createdBodyWithAgent")
          : t("routines.toast.createdBodyDraft"),
        tone: "success",
      });
  ...
  const routineGroups = useMemo(
    () =>
      buildRoutineGroups(sortedRoutines, routineViewState.groupBy, projectById, agentById, {
        noProject: t("routines.list.noProject"),
        unknownProject: t("routines.list.unknownProject"),
        unassigned: t("routines.list.unassigned"),
        unknownAgent: t("routines.list.unknownAgent"),
      }),
  ...
  const recentRunsIssueLinkState = useMemo(
    () =>
      createIssueDetailLocationState(
        t("routines.list.recentRunsBreadcrumb"),
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines.list.tabRoutines") },
            { value: "runs", label: t("routines.list.tabRecentRuns") },
          ]}
        />
            ...
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.list.sort")}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.list.sort")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", "routines.list.sortField.updated"],
                      ["created", "routines.list.sortField.created"],
                      ["lastRun", "routines.list.sortField.lastRun"],
                      ["title", "routines.list.sortField.title"],
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc"
                              ? t("routines.list.sortAsc")
                              : t("routines.list.sortDesc")}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.list.group")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.list.group")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["project", "routines.list.groupBy.project"],
                      ["assignee", "routines.list.groupBy.assignee"],
                      ["none", "routines.list.groupBy.none"],
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("routines.composer.eyebrow")}</p>
              <p className="text-sm text-muted-foreground">
                  ...
                  <InlineEntitySelector
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.composer.assignee")}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  <InlineEntitySelector
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.composer.project")}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines.composer.advancedTitle")}</p>
                    <p className="text-sm text-muted-foreground">{t("routines.composer.advancedSubtitle")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.composer.concurrency")}</p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">{t(`routines.concurrencyDesc.${draft.concurrencyPolicy}`)}</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.composer.catchUp")}</p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">{t(`routines.catchUpDesc.${draft.catchUpPolicy}`)}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routines.composer.footerHint")}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routines.composer.creating") : t("routines.composer.createRoutine")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.composer.createError")}
                </p>
              ) : null}
            </div>
          </div>
