...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("plugins.settings"), href: "/instance/settings/heartbeats" },
      { label: t("plugins.breadcrumb"), href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("plugins.settings.detailsFallback") },
    ]);
            ...
            <section className="space-y-4">
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("plugins.settings.environments.title")}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t("plugins.settings.environments.body", {
                      subject: driverLabel || t("plugins.settings.environments.fallbackSubject"),
                    })}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/environments">
                      <Button variant="outline" size="sm">{t("plugins.settings.environments.openLink")}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("plugins.settings.runtime.title")}
                  </CardTitle>
                  <CardDescription>
                    {t("plugins.settings.runtime.description")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins.settings.runtime.workerProcess")}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.runtime.status")}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.runtime.pid")}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.runtime.uptime")}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.runtime.pendingRpcs")}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("plugins.settings.runtime.crashes")}
                                  </span>
                                  <span className="text-xs">
                                    {t("plugins.settings.runtime.crashesValue", {
                                ...
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("plugins.settings.runtime.lastCrash")}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins.settings.runtime.noWorker")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins.settings.runtime.recentJobRuns")}
                        </h3>
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins.settings.runtime.noJobRuns")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins.settings.runtime.recentWebhookDeliveries")}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins.settings.runtime.noWebhookDeliveries")}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("plugins.settings.runtime.lastChecked", { time: formatTime(dashboardData.checkedAt, { seconds: true }) })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("plugins.settings.runtime.unavailable")}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {t("plugins.settings.logs.title")}
                    </CardTitle>
                    <CardDescription>{t("plugins.settings.logs.description", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="max-h-64 space-y-1 overflow-y-auto font-mono text-xs">
                      {recentLogs.map((entry) => (
                        ...
                        >
                          <span className="shrink-0 text-muted-foreground/50">{formatTime(entry.createdAt, { seconds: true })}</span>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t("plugins.settings.health.title")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("plugins.settings.health.checking")}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("plugins.settings.health.overall")}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("plugins.settings.health.lifecycle")}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("plugins.settings.health.runsWhenReady")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("plugins.settings.details.title")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.details.pluginId")}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.details.pluginKey")}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.details.npmPackage")}</span>
                    <span className="max-w-[170px] truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.details.version")}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("plugins.settings.permissions.title")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("plugins.settings.permissions.none")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("plugins.settings.selectCompanyForFolders")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("plugins.settings.localFolders.title")}</h3>
      </div>
      {error ? (
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
  ...
  const handleSave = useCallback(() => {
    ...
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("plugins.settings.localFolders.pathAbsolute") });
      return;
    }
    setMessage(null);
  ...
  return (
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      ...
      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric
          label={t("plugins.settings.localFolders.configured")}
          value={status?.configured ? t("plugins.settings.localFolders.yes") : t("plugins.settings.localFolders.no")}
          ok={!!status?.configured}
        />
        <FolderStatusMetric
          label={t("plugins.settings.localFolders.readable")}
          value={status?.readable ? t("plugins.settings.localFolders.yes") : t("plugins.settings.localFolders.no")}
          ok={!!status?.readable}
        />
        <FolderStatusMetric
          ...
          value={
            access === "read"
              ? t("plugins.settings.localFolders.notRequested")
              : status?.writable
                ? t("plugins.settings.localFolders.yes")
                : t("plugins.settings.localFolders.no")
          }
