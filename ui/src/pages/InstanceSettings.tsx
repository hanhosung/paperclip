...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.heartbeats.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.heartbeats.updateError"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("settings.heartbeats.unknownError");
        throw new Error(
          failures.length === 1
            ? t("settings.heartbeats.disableOneError", { detail })
            : t("settings.heartbeats.disableManyError", { failed: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.heartbeats.disableAllError"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("settings.heartbeats.loadError")}
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t("settings.heartbeats.company", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(t("settings.heartbeats.confirmDisableAll", { count: enabledCount }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("settings.heartbeats.disablingAll") : t("settings.heartbeats.disableAll")}
          </Button>
        )}
      </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-muted-foreground tabular-nums shrink-0">
                          {t("settings.heartbeats.intervalSeconds", { count: agent.intervalSec })}
                        </span>
                        <span
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("settings.heartbeats.fullConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("settings.heartbeats.disableTimer") : t("settings.heartbeats.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );
