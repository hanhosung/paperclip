...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyData.common.selectFileToPreview")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyData.common.binaryPreviewUnavailable")}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("companyData.import.renamesHeading")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {t("companyData.import.itemCount", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("companyData.import.skipped") : t("companyData.import.skip")}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("companyData.import.confirmed")}
                      </>
                    ) : (
                      t("companyData.import.confirmRename")
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{t("companyData.import.adaptersHeading")}</h3>
          <span className="text-xs text-muted-foreground">
            {t("companyData.common.agentCount", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  <span className={cn(
                    ...
                    {t("companyData.import.agentKind")}
                  </span>
                  ...
                  >
                    ...
                    {t("companyData.import.configureAdapter")}
                  </button>
                </div>
                {isExpanded && (
...
async function readLocalPackageZip(
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(t("companyData.import.selectZipError"));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(t("companyData.import.emptyZipError"));
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText = t("companyData.import.localZipHelp");

  useEffect(() => {
    setBreadcrumbs([
      { label: t("companyData.common.breadcrumbOrgChart"), href: "/org" },
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("companyData.import.noSourceError"));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.import.previewFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.import.previewFailedBody"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyData.import.importCompleteTitle"),
        body: t("companyData.import.importCompleteBody", { name: result.company.name, count: result.agents.length }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.import.importFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.import.importFailedBody"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("companyData.import.packageReadFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.import.packageReadFailedBody"),
      });
    }
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{t("companyData.import.sourceTitle")}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("companyData.import.sourceDescription")}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("companyData.import.sourceGithub") },
              { key: "local", icon: Upload, label: t("companyData.import.sourceLocal") },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("companyData.import.localPackageSummary", {
          ...
          <Field
            label={t("companyData.import.githubUrlLabel")}
            hint={t("companyData.import.githubUrlHint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={t("companyData.import.githubUrlPlaceholder")}
              onChange={(e) => {
        ...
        <Field label={t("companyData.import.targetLabel")} hint={t("companyData.import.targetHint")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("companyData.import.targetNew")}</option>
            <option value="existing">
              {t("companyData.import.targetExisting", { name: selectedCompany?.name ?? "" })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("companyData.import.newCompanyNameLabel")}
            hint={t("companyData.import.newCompanyNameHint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("companyData.import.newCompanyNamePlaceholder")}
            />
          </Field>
        )}

        <Field
          label={t("companyData.import.collisionStrategyLabel")}
          hint={t("companyData.import.collisionStrategyHint")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("companyData.import.collisionRename")}</option>
            <option value="skip">{t("companyData.import.collisionSkip")}</option>
            <option value="replace">{t("companyData.import.collisionReplace")}</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("companyData.import.previewing") : t("companyData.import.previewImport")}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {t("companyData.import.previewHeading")}
              </span>
              <span className="text-muted-foreground">
                {t("companyData.import.selectedSummary", { selected: selectedCount, total: totalFiles })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("companyData.import.conflictCount", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("companyData.import.errorCount", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ? t("companyData.import.importing")
                : t("companyData.import.importButton", { count: selectedCount })}
            </Button>
          </div>
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("companyData.common.packageFiles")}</h2>
              </div>
