...
export function IssueFiltersPopover({
  ...
  const filterButtonLabel = activeFilterCount > 0
    ? t("issueFilter.filtersWithCount", { count: activeFilterCount })
    : t("issueFilter.filter");
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("issueFilter.title")}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("issueFilter.quickFilters")}</span>
            <div className="flex flex-wrap gap-1.5">
              {issueQuickFilterPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                    {t(`issueFilter.preset.${preset.label.toLowerCase()}`)}
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFilter.section.status")}</span>
                <div className="space-y-0.5">
                  {issueStatusOrder.map((status) => (
                    <label key={status} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{t(issueStatusKey(status))}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFilter.section.priority")}</span>
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{t(issuePriorityKey(priority))}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFilter.section.assignee")}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issueFilter.noAssignee")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("issueFilter.me")}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFilter.section.creator")}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("issueFilter.removeCreator", { name: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("issueFilter.searchCreators")}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("issueFilter.noCreators")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFilter.section.project")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFilter.section.labels")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFilter.section.workspace")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFilter.section.visibility")}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("issueFilter.liveRunsOnly")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issueFilter.hideRoutineRuns")}</span>
                  </label>
                ) : null}
              </div>
            </div>
