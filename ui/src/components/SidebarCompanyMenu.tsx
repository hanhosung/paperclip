    ...
    >
      ...
      {isEditing ? (
        <button
          ...
          aria-label={t("chrome.companyMenu.reorder", { name: company.name })}
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={selectedCompany ? t("chrome.companyMenu.openSwitcher", { name: selectedCompany.name }) : t("chrome.companyMenu.openSwitcherGeneric")}
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            {selectedCompany ? <WorkspaceIcon company={selectedCompany} /> : null}
            <span className="truncate text-sm font-bold text-foreground">
              {selectedCompany?.name ?? t("chrome.companyMenu.selectWorkspace")}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          <DropdownMenuLabel className="p-0 text-[11px] font-semibold uppercase text-muted-foreground">
            {t("chrome.companyMenu.switchWorkspace")}
          </DropdownMenuLabel>
          <button
            type="button"
            onClick={(event) => {
          ...
          >
            {isEditingOrder ? t("chrome.companyMenu.done") : t("chrome.companyMenu.edit")}
          </button>
        </div>
        <div className="max-h-96 overflow-y-auto">
          ...
          {orderedCompanies.length === 0 ? (
            <DropdownMenuItem disabled>{t("chrome.companyMenu.noWorkspaces")}</DropdownMenuItem>
          ) : null}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        >
          ...
          <span>{t("chrome.companyMenu.addCompany")}</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {selectedCompany ? t("chrome.companyMenu.invitePeople", { name: selectedCompany.name }) : t("chrome.companyMenu.invitePeopleGeneric")}
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span>{t("chrome.companyMenu.companySettings")}</span>
          </Link>
        </DropdownMenuItem>
        {session?.session ? (
          <>
            ...
            >
              ...
              <span>{signOutMutation.isPending ? t("chrome.companyMenu.signingOut") : t("chrome.companyMenu.signOut")}</span>
            </DropdownMenuItem>
          </>
        ) : null}
      </DropdownMenuContent>
