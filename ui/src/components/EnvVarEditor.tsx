...
export function EnvVarEditor({
  ...
  async function sealRow(index: number) {
    ...
    const name = window.prompt(t("editors.envVar.secretNamePrompt"), suggested)?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(error instanceof Error ? error.message : t("editors.envVar.createSecretFailed"));
    }
  }
    ...
    <div className="space-y-1.5">
        ...
        return (
          <div key={index} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("editors.envVar.keyPlaceholder")}
              value={row.key}
            ...
            >
              <option value="plain">{t("editors.envVar.sourcePlain")}</option>
              <option value="secret">{t("editors.envVar.sourceSecret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">{t("editors.envVar.selectSecret")}</option>
                  {row.secretId && !secrets.some((s) => s.id === row.secretId) ? (
                    <option value={row.secretId}>{t("editors.envVar.missingSecret", { id: row.secretId.slice(0, 8) })}</option>
                  ) : null}
                  {secrets.map((secret) => (
                ...
                <select
                  ...
                  aria-label={t("editors.envVar.version")}
                >
                ...
                <button
                  ...
                  title={t("editors.envVar.newSecretFromPlain")}
                >
                  {t("editors.envVar.newButton")}
                </button>
              </>
            ) : (
              <>
                <input
                  ...
                  placeholder={t("editors.envVar.valuePlaceholder")}
                ...
                <button
                  ...
                  onClick={() => sealRow(index)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("editors.envVar.sealTooltip")}
                >
                  {t("editors.envVar.sealButton")}
                </button>
              </>
            )}
            {!isTrailing ? (
      ...
      {(() => {
        ...
        for (const row of rows) {
          ...
          if (!secret) {
            issues.push({ key: row.key.trim() || row.secretId, reason: t("editors.envVar.reasonMissing") });
        ...
        return (
          <p className="text-[11px] text-amber-700 dark:text-amber-400 inline-flex items-start gap-1">
            ...
            <span>
              {t("editors.envVar.bindingsNeedAttention", { count: issues.length })}{" "}
              ...
              {" "}
              {t("editors.envVar.runsWillFail")}
            </span>
          </p>
        );
      })()}
      <p className="text-[11px] text-muted-foreground/60">
        {t("editors.envVar.hint")}
      </p>
    </div>
  );
}
