...
function getRevisionLabel(
  ...
  const actor = revision.createdByUserId
    ? t("components.documentDiff.actorBoard")
    : revision.createdByAgentId
      ? t("components.documentDiff.actorAgent")
      : t("components.documentDiff.actorSystem");
  return t("components.documentDiff.revisionLabel", {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-[90%] w-full max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 shrink-0">
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-red-400">{t("components.documentDiff.old")}</span>
              <Select
            ...
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-green-500/30 bg-green-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-green-400">{t("components.documentDiff.new")}</span>
              <Select
        ...
        <div className="overflow-auto flex-1 rounded-md border border-border text-xs">
          {!revisions ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("components.documentDiff.loadingRevisions")}</div>
            ...
            <div className="font-mono text-[12px] leading-6">
              <div className="grid grid-cols-[56px_56px_24px_minmax(0,1fr)] border-b border-border/60 bg-muted/30 px-3 py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
                <span>{t("components.documentDiff.old")}</span>
                <span>{t("components.documentDiff.new")}</span>
                <span />
                <span>{t("components.documentDiff.content")}</span>
              </div>
