...
/** i18n key for an issue column label. Resolve with `t()`. */
function issueColumnLabelKey(column: InboxIssueColumn): string {
  return `issues.columns.label.${column}`;
}

/** i18n key for an issue column description. Resolve with `t()`. */
function issueColumnDescriptionKey(column: InboxIssueColumn): string {
  return `issues.columns.description.${column}`;
}

export function issueActivityText(issue: Issue, t?: TranslateFn): string {
  const time = timeAgo(issue.lastActivityAt ?? issue.lastExternalCommentAt ?? issue.updatedAt);
  return t ? t("issues.row.updated", { time }) : `Updated ${time}`;
}
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={t("issues.columns.button")}
        >
          <Columns3 className={iconOnly ? "h-3.5 w-3.5" : "mr-1 h-3.5 w-3.5"} />
          {!iconOnly && t("issues.columns.button")}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
          ...
          >
            <span className="flex flex-col gap-0.5">
              <span className="text-sm font-medium text-foreground">
                {t(issueColumnLabelKey(column))}
              </span>
              <span className="text-xs leading-relaxed text-muted-foreground">
                {t(issueColumnDescriptionKey(column))}
              </span>
            </span>
          </DropdownMenuCheckboxItem>
        ))}
        ...
        >
          ...
          <span className="ml-auto text-xs text-muted-foreground">{t("issues.columns.resetDefaultsHint")}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel = assigneeUserName ?? formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("issues.row.user");
              ...
              {workspaceId && onFilterWorkspace ? (
                <Tooltip>
                  ...
                  <TooltipContent side="top" sideOffset={6}>
                    {t("issues.row.filterByWorkspace")}
                  </TooltipContent>
                </Tooltip>
              ) : (
                workspaceName
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("issues.row.subIssue")}</span>
              )}
            </span>
          );
        }
