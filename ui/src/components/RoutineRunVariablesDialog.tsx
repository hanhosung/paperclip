        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routines.runDialog.agent")}</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("routines.runDialog.agentPlaceholder")}
                noneLabel={t("routines.runDialog.selectAgent")}
                searchPlaceholder={t("routines.runDialog.searchAgents")}
                emptyMessage={t("routines.runDialog.noAgentsFound")}
                disablePortal
                openOnFocus={false}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routines.runDialog.project")}</Label>
              <InlineEntitySelector
                value={selection.projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("routines.runDialog.project")}
                noneLabel={t("routines.runDialog.noProject")}
                searchPlaceholder={t("routines.runDialog.searchProjects")}
                emptyMessage={t("routines.runDialog.noProjectsFound")}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("routines.runDialog.noProject")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="true">{t("routines.runDialog.boolTrue")}</SelectItem>
                    <SelectItem value="false">{t("routines.runDialog.boolFalse")}</SelectItem>
                  </SelectContent>
                </Select>
        ...
        >
          {!selection.assigneeAgentId ? (
            ...
            <p className="mr-auto text-xs text-amber-600">
              {t("routines.runDialog.missing", { fields: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
          ...
          >
            {isPending ? t("routines.runDialog.running") : t("routines.runDialog.runRoutine")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
