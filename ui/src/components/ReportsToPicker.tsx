...
export function ReportsToPicker({
  ...
  disabled = false,
  excludeAgentIds = [],
  disabledEmptyLabel: disabledEmptyLabelProp,
  ...
  const disabledEmptyLabel = disabledEmptyLabelProp ?? t("editors.reportsTo.disabledEmpty");
  const chooseLabel = chooseLabelProp ?? t("editors.reportsTo.choose");
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("editors.reportsTo.unknownManager")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {terminatedManager
                  ? t("editors.reportsTo.reportsToTerminated", { name: current.name })
                  : t("editors.reportsTo.reportsTo", { name: current.name })}
              </span>
            </>
          ) : (
            <>
