...
function windowLabelKey(windowKind: BudgetPolicySummary["windowKind"]): string {
  return windowKind === "lifetime" ? "costs.policy.lifetimeBudget" : "costs.policy.monthlyBudget";
}
...
export function BudgetPolicyCard({
  ...
  const limitLabel = summary.amount > 0
    ? t("costs.policy.percentOfLimit", { percent: summary.utilizationPercent })
    : t("costs.policy.noCap");
  const softAlertLabel = `${t("costs.policy.softAlert", { percent: summary.warnPercent })}${
    summary.paused && summary.pauseReason
      ? ` · ${t("costs.policy.pauseSuffix", { reason: summary.pauseReason })}`
      : ""
  }`;
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("costs.policy.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">{limitLabel}</div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("costs.policy.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("costs.policy.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">{softAlertLabel}</div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("costs.policy.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("costs.policy.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("costs.policy.pausedProject")
          : t("costs.policy.pausedScope")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("costs.policy.saving") : summary.amount > 0 ? t("costs.policy.updateBudget") : t("costs.policy.setBudget")}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
              {t(`costs.scope.${summary.scopeType}`)}
            </div>
          ...
          >
            ...
            {summary.paused ? t("costs.policy.statusPaused") : summary.status === "warning" ? t("costs.policy.statusWarning") : summary.status === "hard_stop" ? t("costs.policy.statusHardStop") : t("costs.policy.statusHealthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("costs.policy.invalidAmount")}</p>
        ) : null}
      </div>
    );
  }
