...
/** i18n key for an approval type label (`hire_agent`, `request_board_approval`, …). */
export function typeLabelKey(type: string): string {
  return `approvals.type.${type}`;
}
...
export function approvalLabel(
  ...
  const base = APPROVAL_TYPES.has(type) ? t(typeLabelKey(type)) : type;
    ...
    <div className="mt-3 space-y-1.5 text-sm">
      ...
      {(budgetAmount !== null || observedAmount !== null) ? (
        <div className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          {t("approvals.payload.limitObserved", {
    ...
    <div className="mt-4 space-y-3.5 text-sm">
      {title && (
        <div className="space-y-1">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("approvals.payload.title")}</p>
          <p className="font-medium leading-6 text-foreground">{title}</p>
        </div>
      )}
      {summary && (
        <div className="space-y-1">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("approvals.payload.summary")}</p>
      ...
      {nextActionOnApproval && (
        <div className="rounded-lg border border-border/60 bg-background/60 px-3.5 py-3">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("approvals.payload.onApproval")}</p>
          <p className="mt-1 leading-6 text-foreground">{nextActionOnApproval}</p>
        </div>
      )}
      {risks.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("approvals.payload.risks")}</p>
