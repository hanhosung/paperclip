...
import { fieldKey, issueStatusKey } from "../lib/enum-labels";
import {
    ...
    <div className="border border-border bg-background p-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div
            role="progressbar"
            aria-label={t("a11y.subIssuesProgress")}
            aria-valuemin={0}
          ...
          >
            {statusEntries.map(({ status, count }) => (
              <span
                ...
                title={`${t(issueStatusKey(status))}: ${count}`}
                aria-hidden="true"
              />
            ))}
          </div>
...
export function IssuesList({
  ...
  const createActionLabel = createIssueLabel
    ? t("issues.createActionNamed", { label: createIssueLabel })
    : t("issues.createAction");
  const createButtonLabel = createIssueLabel
    ? t("issues.createButtonNamed", { label: createIssueLabel })
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {(["status", "priority", "assignee", "project", "workspace", "parent", "none"] as const).map((value) => (
                    ...
                    >
                      <span>{value === "none" ? t("common.none") : t(fieldKey(value))}</span>
                          ...
                          <>
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                aria-label={t("a11y.needsNextStep")}
                                title={t("a11y.needsNextStepTitle")}
                              >
