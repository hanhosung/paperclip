        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("costs.unit.in")}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("costs.unit.out")}
              {(totalApiRuns > 0 || totalSubRuns > 0) && (
                <span className="ml-1.5">
                  ·{" "}
                  {totalApiRuns > 0 && t("costs.providerCard.apiRuns", { count: totalApiRuns })}
                  {totalApiRuns > 0 && totalSubRuns > 0 && " / "}
                  {totalSubRuns > 0 && t("costs.providerCard.subRuns", { count: totalSubRuns })}
                  {" "}
                  {t("costs.providerCard.runsSuffix")}
                </span>
              )}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {hasBudget && (
          <div className="space-y-3">
            <QuotaBar
              label={t("costs.quota.periodSpend")}
              percentUsed={budgetPct}
              leftLabel={formatCents(totalCostCents)}
              rightLabel={t("costs.quota.percentOfAllocation", { percent: Math.round(budgetPct) })}
              showDeficitNotch={showDeficitNotch}
            />
            <QuotaBar
              label={t("costs.providerCard.thisWeek")}
              percentUsed={weekPct}
              leftLabel={formatCents(weekSpendCents)}
              rightLabel={t("costs.providerCard.perWeek", { amount: formatCents(Math.round(weeklyBudgetShare)) })}
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("costs.providerCard.rollingWindows")}
              </p>
                    ...
                    <div key={w} className="space-y-1">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground font-mono flex-1">
                          {t("costs.unit.tok", { tokens: formatTokens(tokens) })}
                        </span>
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("costs.providerCard.subscription")}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{totalSubRuns}</span> {t("costs.providerCard.runsWord")}
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span> {t("costs.providerCard.total")}
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("costs.unit.in")}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("costs.unit.out")}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("costs.providerCard.subShare", { percent: Math.round(subSharePct) })}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <span className="text-[11px] text-muted-foreground truncate block">
                          {providerDisplayName(row.biller)} · {t(billingTypeKey(row.billingType))}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 shrink-0 tabular-nums text-xs">
                        <span className="text-muted-foreground">
                          {t("costs.unit.tok", { tokens: formatTokens(rowTokens) })}
                        </span>
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("costs.providerCard.tokenShare", { percent: Math.round(tokenPct) })}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("costs.providerCard.costShare", { percent: Math.round(costPct) })}
                      />
                    </div>
                  </div>
                );
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {t("costs.providerCard.subscriptionQuota")}
                </p>
