      ...
      >
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={t("projects.newDialog.descriptionPlaceholder")}
            bordered={false}
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("projects.newDialog.repoUrlLabel")}</label>
              <span className="text-xs text-muted-foreground/50">{t("projects.newDialog.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("projects.newDialog.repoUrlTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
              ...
              placeholder={t("projects.newDialog.repoUrlPlaceholder")}
            />
          </div>

          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("projects.newDialog.localFolderLabel")}</label>
              <span className="text-xs text-muted-foreground/50">{t("projects.newDialog.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("projects.newDialog.localFolderTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
          ...
          {selectedGoals.map((goal) => (
            ...
            >
              ...
              <button
                ...
                aria-label={t("projects.newDialog.removeGoal", { title: goal.title })}
                type="button"
              >
                <X className="h-3 w-3" />
              </button>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("projects.newDialog.createFailed")}</p>
          ) : (
            <span />
          )}
          <Button
