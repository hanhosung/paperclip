    ...
    <div className="mb-3 rounded-lg border border-border bg-muted/30 px-3 py-2">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="text-sm font-medium text-foreground">{t("issueNotice.monitor.title")}</div>
          <div className="text-xs text-muted-foreground">
            {t("issueNotice.monitor.nextCheck", {
          ...
          {monitor.attemptCount > 0 ? (
            <div className="mt-1 text-xs text-muted-foreground">
              {t("issueNotice.monitor.attempt", { count: monitor.attemptCount })}
            </div>
          ) : null}
        </div>
        {onCheckNow ? (
          ...
          >
            {checkingNow ? t("issueNotice.monitor.checking") : t("issueNotice.monitor.checkNow")}
          </Button>
        ) : null}
      </div>
    </div>
