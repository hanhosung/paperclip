...
function blockedAttentionLabel(
  ...
  if (!blockerAttention || blockerAttention.state === "none") return t("misc.statusIcon.blocked");

  if (blockerAttention.reason === "active_child") {
    const count = blockerAttention.coveredBlockerCount;
    if (count === 1 && blockerAttention.sampleBlockerIdentifier) {
      return t("misc.statusIcon.activeChildSample", { id: blockerAttention.sampleBlockerIdentifier });
    }
    return t("misc.statusIcon.activeChild", { count });
  }

  if (blockerAttention.reason === "active_dependency") {
    const count = blockerAttention.coveredBlockerCount;
    if (count === 1 && blockerAttention.sampleBlockerIdentifier) {
      return t("misc.statusIcon.activeDependencySample", { id: blockerAttention.sampleBlockerIdentifier });
    }
    return t("misc.statusIcon.activeDependency", { count });
  }
  ...
  if (blockerAttention.reason === "stalled_review") {
    ...
    if (count === 1 && leaf) return t("misc.statusIcon.stalledReviewSample", { id: leaf });
    return t("misc.statusIcon.stalledReview", { count });
  }

  if (blockerAttention.reason === "attention_required") {
    const count = blockerAttention.attentionBlockerCount || blockerAttention.unresolvedBlockerCount;
    const attentionCopy = t("misc.statusIcon.attentionCopy", { count });
    const coveredCount = blockerAttention.coveredBlockerCount;
    if (coveredCount > 0) {
      return t("misc.statusIcon.attentionFullCovered", { attention: attentionCopy, covered: coveredCount });
    }
    return t("misc.statusIcon.attentionFull", { attention: attentionCopy });
  }

  return t("misc.statusIcon.blocked");
}
...
export function StatusIcon({ status, blockerAttention, onChange, className, showLabel }: StatusIconProps) {
  ...
  const ariaLabel = status === "blocked" ? blockedAttentionLabel(t, blockerAttention) : t(issueStatusKey(status));
  ...
  if (!onChange) return showLabel ? <span className="inline-flex items-center gap-1.5">{circle}<span className="text-sm">{t(issueStatusKey(status))}</span></span> : circle;
