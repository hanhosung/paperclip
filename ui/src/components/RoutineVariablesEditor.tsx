      ...
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {t("routines.variables.detectedHint", { placeholder: "{{name}}" })}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routines.variables.promptHint")}
              </span>
            </div>
                ...
                >
                  ...
                  <SelectContent>
                    {variableTypes.map((type) => (
                      <SelectItem key={type} value={type}>{t(`routines.variables.type_${type}`)}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="true">{t("routines.variables.boolTrue")}</SelectItem>
                      <SelectItem value="false">{t("routines.variables.boolFalse")}</SelectItem>
                    </SelectContent>
                  </Select>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        placeholder={t("routines.variables.optionsPlaceholder")}
                      />
                    </div>
                    <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? t("routines.variables.numberPlaceholder") : t("routines.variables.defaultValuePlaceholder")}
                  />
                )}
              </div>
            </div>
...
const BUILTIN_VARIABLE_DOCS: BuiltinVariableDoc[] = [
  {
    name: "date",
    example: "2026-04-28",
    descriptionKey: "routines.variables.builtinDate",
  },
  {
    name: "timestamp",
    example: "April 28, 2026 at 12:17 PM UTC",
    descriptionKey: "routines.variables.builtinTimestamp",
  },
];
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t("routines.variables.hintInline", { placeholder: "{{variable_name}}" })}
        </span>
        <button
          type="button"
          ...
          aria-label={t("routines.variables.hintHelpAria")}
        >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routines.variables.helpTitle")}</DialogTitle>
            <DialogDescription>
              {t("routines.variables.helpDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routines.variables.customTitle")}
              </h3>
              <p className="text-muted-foreground">
                {t("routines.variables.customBody", { code: "{{variable_name}}" })}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("routines.variables.customRule1")}</li>
                <li>{t("routines.variables.customRule2")}</li>
                <li>{t("routines.variables.customRule3")}</li>
              </ul>
            </section>

            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routines.variables.builtinTitle")}
              </h3>
              <p className="text-muted-foreground">
                {t("routines.variables.builtinBody")}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("routines.variables.colPlaceholder")}</th>
                      <th className="px-3 py-2 font-medium">{t("routines.variables.colExample")}</th>
                      <th className="px-3 py-2 font-medium">{t("routines.variables.colDescription")}</th>
                    </tr>
                  </thead>
