...
function RunCardRecoveryChip({ action }: { action: IssueRecoveryAction }) {
  ...
  return (
    <span
      ...
      title={t("components.recovery.chipTitle", { label: toneLabel })}
      className={cn(
...
export function ActiveAgentsPanel({
  ...
  const { t } = useTranslation();
  const resolvedTitle = title ?? t("components.activeAgents.defaultTitle");
  const resolvedEmptyMessage = emptyMessage ?? t("components.activeAgents.emptyMessage");
      ...
      {showMoreLink && hiddenRunCount > 0 && (
        <div className="mt-3 flex justify-end text-xs text-muted-foreground">
          <Link to="/dashboard/live" className="hover:text-foreground hover:underline">
            {t("components.activeAgents.moreRuns", { count: hiddenRunCount })}
          </Link>
        </div>
      )}
    </div>
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive
                ? t("components.activeAgents.liveNow")
                : run.finishedAt
                  ? t("components.activeAgents.finished", { time: relativeTime(run.finishedAt) })
                  : t("components.activeAgents.started", { time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
