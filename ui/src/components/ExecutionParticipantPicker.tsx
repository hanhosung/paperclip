...
export function ExecutionParticipantPicker({
  ...
  const participantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) return userLabel(value.slice("user:".length)) ?? t("editors.participantPicker.user");
    return value;
  };
  ...
  const label = stageType === "review"
    ? t("editors.participantPicker.reviewers")
    : t("editors.participantPicker.approvers");
  const searchPlaceholder = stageType === "review"
    ? t("editors.participantPicker.searchReviewers")
    : t("editors.participantPicker.searchApprovers");
  const noneLabel = stageType === "review"
    ? t("editors.participantPicker.noReviewers")
    : t("editors.participantPicker.noApprovers");
        ...
        <div className="max-h-48 overflow-y-auto overscroll-contain">
          ...
          {currentUserId && (
            ...
            >
              ...
              {t("editors.participantPicker.assignToMe")}
            </button>
          )}
          {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
            ...
            >
              <User className="h-3 w-3 shrink-0 text-muted-foreground" />
              {creatorUserLabel ?? t("editors.participantPicker.requester")}
            </button>
          )}
          {otherUserOptions
            .filter((option) => {
