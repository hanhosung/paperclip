...
export function IssueAssignedBacklogNotice({
  ...
  const assigneeLabel = assigneeAgent?.name ?? t("issueChat.backlogNotice.assigneeFallback");
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          <p className="leading-5">
            <span className="font-medium">{t("issueChat.backlogNotice.parked")}</span> —{" "}
            <span className="font-medium">{assigneeLabel}</span> {t("issueChat.backlogNotice.parkedBodyBefore")}{" "}
            <code className="rounded bg-amber-100 px-1 py-0.5 text-[12px] dark:bg-amber-400/15">todo</code> {t("issueChat.backlogNotice.parkedBodyOr")}{" "}
            <code className="rounded bg-amber-100 px-1 py-0.5 text-[12px] dark:bg-amber-400/15">in_progress</code>.
          </p>
          {assigneeAgent ? (
            <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
              {t("issueChat.backlogNotice.agentHint")}
            </p>
          ) : null}
          {onResume ? (
            <div className="pt-0.5">
              ...
              >
                {resuming ? t("issueChat.backlogNotice.resuming") : t("issueChat.backlogNotice.resume")}
              </Button>
            </div>
          ) : null}
        </div>
