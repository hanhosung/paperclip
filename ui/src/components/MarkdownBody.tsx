...
function MarkdownIssueLink({
  ...
  const issueLabel = title !== identifier
    ? t("editors.markdownBody.issueLabelWithTitle", { identifier, title })
    : t("editors.markdownBody.issueLabel", { identifier });
...
function CodeBlock({
  ...
  const label = failed
    ? t("editors.markdownBody.copyFailed")
    : copied
      ? t("editors.markdownBody.copied")
      : t("editors.markdownBody.copy");
  ...
  return (
    <div className="paperclip-markdown-codeblock">
      ...
      <button
        type="button"
        onClick={handleCopy}
        aria-label={t("editors.markdownBody.copyCode")}
        title={label}
      ...
      .catch((err) => {
        ...
        const message =
          err instanceof Error && err.message
            ? err.message
            : t("editors.markdownBody.mermaidRenderFailed");
        setError(message);
      });
      ...
      {svg ? (
        ...
        <>
          <p className={cn("paperclip-mermaid-status", error && "paperclip-mermaid-status-error")}>
            {error
              ? t("editors.markdownBody.mermaidError", { error })
              : t("editors.markdownBody.mermaidRendering")}
          </p>
...
export function MarkdownBody({
  ...
  const components: Components = {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="paperclip-markdown-table-scroll" role="region" aria-label={t("editors.markdownBody.scrollableTable")} tabIndex={0}>
