...
/* Thinking-effort / mode option lists. `labelKey` is an i18n key resolved by
 * the dropdown with its own `t()`. */
const codexThinkingEffortOptions = [
  { id: "", labelKey: "agentConfig.option.auto" },
  { id: "minimal", labelKey: "agentConfig.option.minimal" },
  { id: "low", labelKey: "agentConfig.option.low" },
  { id: "medium", labelKey: "agentConfig.option.medium" },
  { id: "high", labelKey: "agentConfig.option.high" },
  { id: "xhigh", labelKey: "agentConfig.option.xHigh" },
] as const;
...
const openCodeThinkingEffortOptions = [
  ...
  { id: "high", labelKey: "agentConfig.option.high" },
  { id: "xhigh", labelKey: "agentConfig.option.xHigh" },
  { id: "max", labelKey: "agentConfig.option.max" },
] as const;

const cursorModeOptions = [
  { id: "", labelKey: "agentConfig.option.auto" },
  { id: "plan", labelKey: "agentConfig.option.plan" },
  { id: "ask", labelKey: "agentConfig.option.ask" },
] as const;
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : t("agentConfig.msg.refreshModelsFailed"));
    } finally {
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("agentConfig.msg.unsavedChanges")}</span>
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isCreate && props.isSaving}
            >
              {!isCreate && props.isSaving ? t("agentConfig.button.saving") : t("agentConfig.button.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfig.section.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfig.section.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentConfig.field.name")} hint={help.name}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={t("agentConfig.placeholder.agentName")}
              />
            </Field>
            <Field label={t("agentConfig.field.title")} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("agentConfig.placeholder.title")}
              />
            </Field>
            <Field label={t("agentConfig.field.reportsTo")} hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel={t("agentConfig.reportsTo.chooseManager")}
              />
            </Field>
            <Field label={t("agentConfig.field.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("agentConfig.placeholder.capabilities")}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("agentConfig.field.promptTemplate")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("agentConfig.placeholder.promptTemplate", {
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("agentConfig.msg.promptTemplateWarning")}
                </div>
              </>
            )}
          </div>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfig.section.execution")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfig.section.execution")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("agentConfig.field.defaultEnvironment")}
              hint="agentConfig.hint.defaultEnvironment"
            >
              <select
                className={inputClass}
              ...
              >
                <option value="">{t("agentConfig.option.companyDefaultLocal")}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("agentConfig.section.adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("agentConfig.section.adapter")}</span>
          }
          {showInlineAdapterTestEnvironmentButton && (
            <Button
              ...
              disabled={testEnvironmentDisabled}
            >
              {testEnvironment.isPending ? t("agentConfig.button.testing") : t("agentConfig.button.test")}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("agentConfig.field.adapterType")} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showInlineAdapterTestEnvironmentFeedback && testEnvironment.error && (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              {testEnvironment.error instanceof Error
                ? testEnvironment.error.message
                : t("agentConfig.msg.environmentTestFailed")}
            </div>
          )}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("agentConfig.field.workingDirectoryDeprecated")} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfig.section.permissions")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfig.section.permissions")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("agentConfig.field.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agentConfig.field.primaryModel")}</div>
              )}
              <ModelDropdown
                models={models}
                ...
                detectModelLabel={t("agentConfig.button.detectModel")}
                emptyDetectHint={t("agentConfig.msg.detectModelHint")}
              />
                ...
                <p className="text-xs text-destructive">
                  {refreshModelsError
                    ?? (fetchedModelsError instanceof Error
                      ...
                      : t("agentConfig.msg.loadModelsFailed"))}
                </p>
              )}
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("agentConfig.msg.openCodeDiscoveryLocalOnly", { environment: currentDefaultEnvironment.name })}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("agentConfig.msg.codexMinimalSearchWarning")}
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agentConfig.field.bootstrapPromptLegacy")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("agentConfig.placeholder.bootstrapPrompt")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agentConfig.msg.bootstrapPromptWarning")}
                  </div>
                </>
              )}
              ...
              <Field label={t("agentConfig.field.extraArgs")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("agentConfig.placeholder.extraArgs")}
                />
              </Field>

              <Field label={t("agentConfig.field.envVars")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("agentConfig.field.timeoutSec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agentConfig.field.graceSec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("agentConfig.section.runPolicy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("agentConfig.section.runPolicy")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("agentConfig.field.heartbeatOnInterval")}
              hint={help.heartbeatInterval}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("agentConfig.hint.secondsUnit")}
              numberPrefix={t("agentConfig.hint.heartbeatRunEvery")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("agentConfig.section.advancedRunPolicy")}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("agentConfig.field.wakeOnDemand")}
              ...
              />
              <Field label={t("agentConfig.field.cooldownSec")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("agentConfig.field.maxConcurrentRuns")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("agentConfig.field.continueAfterMaxTurn")}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("agentConfig.field.continuationAttempts")} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("agentConfig.field.continuationDelaySec")} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  ...
  const statusLabel =
    result.status === "pass"
      ? t("agentConfig.envTest.passed")
      : result.status === "warn"
        ? t("agentConfig.envTest.warnings")
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{t("agentConfig.msg.comingSoon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ExperimentalBadge() {
  ...
  return (
    <span className="shrink-0 rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium leading-none text-amber-700 dark:text-amber-200">
      {t("agentConfig.experimentalBadge")}
    </span>
  );
}
...
function ModelDropdown({
  ...
  return (
    <Field label={t("agentConfig.field.model")} hint={help.model}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ...
                : value
                  || (allowDefault
                    ? (defaultLabel ?? t("agentConfig.option.default"))
                    : required
                      ? t("agentConfig.msg.selectModelRequired")
                      : t("agentConfig.msg.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("agentConfig.placeholder.searchModelsCreatable") : t("agentConfig.placeholder.searchModels")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel
                ? t("agentConfig.button.detecting")
                : detectedModel
                  ? t("agentConfig.button.reDetectFromConfig")
                  : (detectModelLabel ?? t("agentConfig.button.detectFromConfig"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("agentConfig.button.refreshing") : t("agentConfig.button.refreshModels")}
            </button>
          )}
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {canCreateManualModel && (
              ...
              >
                <span>{t("agentConfig.msg.useManualModel")}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("agentConfig.msg.noModelDetectedManual"))
                    : t("agentConfig.msg.noModelsFound")}
                </p>
              </div>
            )}
          </div>
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? t("agentConfig.msg.cheapModelAdapterDefault", { model: adapterDefaultModel })
    : t("agentConfig.msg.cheapModelNoAdapterDefault");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agentConfig.field.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">
            {t("agentConfig.msg.cheapModelDescription")}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          {t("agentConfig.msg.cheapModelFallback", { model: adapterDefaultModel })}
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {t("agentConfig.msg.cheapModelNoneSelected")}
        </p>
      ) : null}
    </div>
  );
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("agentConfig.field.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected ? t(selected.labelKey) : t("agentConfig.option.auto")}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
