...
/** i18n key for the workspace kind badge. */
function workspaceKindKey(kind: ProjectWorkspaceSummary["kind"]) {
  return kind === "execution_workspace"
    ? "workspaces.summaryCard.executionWorkspace"
    : "workspaces.summaryCard.projectWorkspace";
}
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
                {t(workspaceKindKey(summary.kind))}
              </span>
              <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                {t("workspaces.summaryCard.updated", { time: timeAgo(summary.lastUpdatedAt) })}
              </span>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("workspaces.summaryCard.serviceCount", {
              ...
              {summary.executionWorkspaceStatus ? (
                <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                  {t(executionWorkspaceStatusKey(summary.executionWorkspaceStatus), {
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices
                  ? t("workspaces.summaryCard.stopServices")
                  : t("workspaces.summaryCard.startServices")}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed"
                  ? t("workspaces.summaryCard.retryClose")
                  : t("workspaces.summaryCard.closeWorkspace")}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("workspaces.summaryCard.branch")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("workspaces.summaryCard.branchCopied")}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={t("workspaces.summaryCard.copyBranch")}
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("workspaces.summaryCard.path")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("workspaces.summaryCard.pathCopied")}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={t("workspaces.summaryCard.copyPath")}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={t("workspaces.summaryCard.pathCopied")}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("workspaces.summaryCard.service")}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              {t("workspaces.summaryCard.linkedIssues")}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {t("workspaces.summaryCard.moreIssues", { count: hiddenIssueCount })}
                </Link>
              ) : null}
            </div>
          </div>
